...
namespace Standard.Data.Japson
{
    ...
    /// <summary>
    /// This class contains methods used to tokenize JAPSON configuration strings.
    /// </summary>
    public class JapsonTokenizer : Tokenizer
    {
        private const string NotInUnquotedKey = "$\"{}'[]:=,#`^?!@*&\\.";
        private const string NotInUnquotedText = "$\"{}'[]:=,#`^?!@*&\\";

        /// <summary>
        /// Initializes a new instance of the <see cref="JapsonTokenizer"/> class.
        /// </summary>
        /// <param name="text">The string that contains the text to tokenize.</param>
        public JapsonTokenizer(string text)
            : base(text)
        {
        }
        ...
        public JapsonToken PullNext()
        {
            ...
            if (EOF)
                return new JapsonToken(TokenType.EOF, Index, 0);

            throw new JapsonTokenizerException(string.Format(RS.ErrUnknownToken, GetHelpTextAtIndex(start)));
        }
        ...
        public JapsonToken PullArrayEnd()
        {
            int start = Index;
            if (!IsArrayEnd())
                throw new JapsonTokenizerException(string.Format("Expected end of array {0}", GetHelpTextAtIndex(start)));

            Take();
            return new JapsonToken(TokenType.ArrayEnd, start, Index - start);
        }
        ...
        public JapsonToken PullArrayStart()
        {
            int start = Index;
            Take();
            return new JapsonToken(TokenType.ArrayStart, Index, Index - start);
        }
        ...
        public JapsonToken PullDot()
        {
            int start = Index;
            Take();
            return new JapsonToken(TokenType.Dot, start, Index - start);
        }
        ...
        public JapsonToken PullComma()
        {
            int start = Index;
            Take();
            return new JapsonToken(TokenType.Comma, start, Index - start);
        }
        ...
        public JapsonToken PullStartOfObject()
        {
            int start = Index;
            Take();
            return new JapsonToken(TokenType.ObjectStart, start, Index - start);
        }
        ...
        public JapsonToken PullObjectEnd()
        {
            int start = Index;
            if (!IsObjectEnd())
                throw new JapsonTokenizerException(string.Format(RS.ErrExpectEndOfObject, GetHelpTextAtIndex(Index)));

            Take();
            return new JapsonToken(TokenType.ObjectEnd, start, Index - start);
        }
        ...
        public JapsonToken PullAssignment()
        {
            int start = Index;
            Take();
            return new JapsonToken(TokenType.Assign, start, Index - start);
        }
        ...
        public JapsonToken PullComment()
        {
            int start = Index;
            PullRestOfLine();
            return new JapsonToken(TokenType.Comment, start, Index - start);
        }
        ...
        public JapsonToken PullUnquotedKey()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            ...
            return JapsonToken.Key((sb.ToString().Trim()), start, Index - start);
        }
        ...
        public JapsonToken PullTripleQuotedText()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take(3);
            ...
            if (!Matches("\""))
                throw new JapsonTokenizerException(string.Format(RS.ErrExpectEndOfTripleQuote, GetHelpTextAtIndex(Index)));

            Take(3);
            return JapsonToken.LiteralValue(sb.ToString(), start, Index - start);
        }
        ...
        /// </summary>
        /// <returns>A <see cref="TokenType.LiteralValue"/> token from the tokenizer's current position.</returns>
        public JapsonToken PullQuotedText()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take();
            ...
            if (!Matches("\""))
                throw new JapsonTokenizerException(string.Format(RS.ErrExpectEndOfQuote, GetHelpTextAtIndex(Index)));
        ...
        /// </summary>
        /// <returns>A <see cref="TokenType.LiteralValue"/> token from the tokenizer's current position.</returns>
        public JapsonToken PullSingleQuotedText()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take();
        ...
        /// </summary>
        /// <returns>A <see cref="TokenType.Key"/> token from the tokenizer's current position.</returns>
        public JapsonToken PullQuotedKey()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take();
            ...
            Take();
            return JapsonToken.Key(sb.ToString(), start, Index - start);
        }
        ...
        /// </summary>
        /// <returns>A <see cref="TokenType.Key"/> token from the tokenizer's current position.</returns>
        public JapsonToken PullSingleQuotedKey()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take();
        ...
        public JapsonToken PullInclude()
        {
            int start = Index;
            Take("include".Length);
            PullWhitespaceAndComments();
            JapsonToken rest = PullQuotedText();
            string unQuote = rest.Value;
            return JapsonToken.Include(unQuote, start, Index - start);
        }
...
#endif
                default:
                    throw new JapsonTokenizerException(string.Format(RS.ErrUnknownEscapeCode, escaped, GetHelpTextAtIndex(start)));
            }
        }
        ...
        public JapsonToken PullValue()
        {
            ...
            throw new JapsonTokenizerException(string.Format(RS.ErrUnexpectedValue, "null, array, quoted text, unquoted text, triple quoted text, object, end of array", GetHelpTextAtIndex(start)));
        }
        ...
        public JapsonToken PullSubstitution()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            Take(2);
            ...
            Take();
            return JapsonToken.Substitution(sb.ToString(), start, Index - start);
        }
        ...
        public JapsonToken PullSpaceOrTab()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            while (IsSpaceOrTab())
        ...
        private JapsonToken PullUnquotedText()
        {
            int start = Index;
            StringBuilder sb = new StringBuilder();
            while (!EOF && IsUnquotedText())
        ...
        /// </exception>
        public JapsonToken PullSimpleValue()
        {
            int start = Index;
            if (IsSpaceOrTab())
            ...
            throw new JapsonTokenizerException(string.Format(RS.ErrExpectSimpleValue, GetHelpTextAtIndex(start)));
        }
