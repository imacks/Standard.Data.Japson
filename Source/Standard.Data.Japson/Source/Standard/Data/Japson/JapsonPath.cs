...
{
    /// <summary>
    /// Split a Japson path expression into its constituting parts.
    /// </summary>
    /// <remarks>
    /// Japson path expressions are dot separated keys. However, if the key itself contains a dot, that key
    /// must be surrounded with double or single quotes. If the key also contains single or double quote characters,
    /// it must also be escaped with the backslash character.
    ///
    /// <examples>
    /// The expression ```foo.bar.tar``` means nested elements in the structure ```foo -> bar -> tar```.
    ///
    ...
    /// foo.'b.a\'r'.tar
    /// ````
    /// Note that the backslash symbol is only interpreted as an escape sequence if the character immediately after it
    /// is a quote character that must be escaped in the context. For example, ```'b.a\\r'``` is interpreted as 
    /// ```b.a\\r```, and ```'b.a.\"r'``` is interpreted as ```b.a.\"r```.  
    /// </examples>
    /// </remarks>
    internal class JapsonPath
    {
        private readonly string _pathStr;
        private string[] _pathElements;

        public JapsonPath(string pathStr)
        {
            _pathStr = pathStr;
        }
